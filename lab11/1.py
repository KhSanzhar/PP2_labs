...
def limit(cursor):
    ...
    x=input()
    cursor.execute('''create or replace function limit_(count_ int) 
    RETURNS TABLE( 	name varchar, 	phone int  )
    as $$ begin  
    return query select * from contacts limit count_; 
    end $$ 
    language plpgsql;''')
...
def multiadd(conn,cursor):
    ...
    records = cursor.fetchall()
    not_cor=[]
    with open('C:\\Users\\Admin\\Desktop\\Python\\lab11\\sample.txt') as csv_file:
        ...
        for i in csv_reader:
            ...
            if not check_number(i[0]):
                ...
                check=False
            if check:
                cursor.execute('''CREATE or replace FUNCTION new_acc(new_name varchar, new_phone integer) 
                RETURNS void 
                AS   $BODY$       
                BEGIN         
                ...
                cursor.execute('''create or replace procedure add_contacts(old_name varchar(255),new_number int ) 
                language plpgsql 
                as $$ begin   
                update contacts  set phone = new_number   where name=old_name;  
                end;$$;
                ''')
...
def delete(conn,cursor):
    ...
    if x == '1':
        x=int(input())
        cursor.execute('''create or replace procedure delete_(   old_name integer ) 
        language plpgsql
        as $$ begin   
        delete from contacts  where phone=old_name; 
        end;$$;
        ''')
        ...
        x=input()
        cursor.execute('''create or replace procedure delete_(   old_name varchar(255) ) 
        language plpgsql 
        as $$ begin   
        delete from contacts  where name=old_name; 
        end;$$; 
        ''')
        cursor.execute('call delete_(%s)',(x,))
        conn.commit()
...
def show(cursor):
    print('только номера')
    cursor.execute('''create or replace function onlynumbers() 
    RETURNS TABLE(	phone int ) 
    as $$ begin  
    return query select contacts.phone from contacts; 
    end $$ 
    language plpgsql;  
    ...
    print(show)
    print('имя и номер')
    cursor.execute('''create or replace function ALL_() RETURNS TABLE( 	name varchar, 	phone int )
    as $$ begin  
    return query 
